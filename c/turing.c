...
void print_transition(Transition *t) {
  ...
  printf("current:%c tape:%c new state:%c new tape:%c direction %s\n",
         t->current_state, t->tape_symbol, t->new_state, t->new_tape_symbol,
         (t->dir == LEFT) ? s1 : s2);
}
...
TM *createTM(char *input) {
  ...
  if (!is_valid_input(m->input_alpha, buffer)) {
    printf("Error! Input contains some invalid characters that don't match the "
           "input alphabet!\n");
    exit(1);
  }
...
void simulate(TM *m) {
  ...
  while (1) {
    ...
    current_tape_symbol =
        (current_tape == NULL || current_tape->content == '\0')
            ? blank
            : current_tape->content;
    current_transition =
        find_transition(m->transition, current_state, current_tape_symbol);
